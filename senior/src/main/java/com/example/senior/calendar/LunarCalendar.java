...
public class LunarCalendar {
    ...
    public String animalsYear(int year) {
        final String[] Animals = new String[]{
                "鼠", "牛", "虎", "兔子", "龙", "蛇", "马", "羊", "猴", "鸡", "狗", "猪"};
        return Animals[(year - 4) % 12];
    }

    //传入月日，传回干支，0 = 甲子
    private static String cyclicalm(int num) {
        final String[] Gan = new String[]{ "甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "葵"};
    ...
    /*
    *  传入y年m月d日对应的农历，yearCy13：农历年与1864的相差数？ monCy14:从1900年1月31日以来，闰年数
    *  dayCy15：与1900年1月31日相差的天数,再加40？
    *
    *  isday：这个参数为false---日期为节假日时，阴历日历就返回节假日，true---不管日期是否为节假日依然返回这天对相应的阴历日期
    *
    * */
    @SuppressLint("DefaultLocale")
    public CalendarTransfer getLunarDate(int year_log, int month_log, int day_log, boolean isday) {
        int yearCyl, monCy1, dayCy1;
        String nowadays;
        Date baseDate = null;
        Date nowaday = null;
        try{
        ...
        //求出和1900年1月31日相差的天数
        int offset = (int) ((nowaday.getTime() - ((Date) baseDate).getTime()) / 86400000L);
        dayCy1 = offset + 40;
        monCy1 = 14;

        //用offset减去每年农历的天数，计算当天是农历第几天
        //iYear最终结果是农历的年份，offse是当年的第几天
        ...
        //用当年的天数offset，逐个减去每月（农历）的天数，求出当天的本月的第几天
    ...
    public CalendarTransfer getSubDate(CalendarTransfer lastTrans, int year_log, int month_log, int day_log, int weekday, boolean isday) {
        ...
        String day_result = "";
        String day_name;
        if (lastTrans.lunar_month == 0 || lastTrans.lunar_day == 0 || lastTrans.lunar_day == 29 || lastTrans.lunar_day == 30) {
        ...
        if (!isday) {
            ...
            for (int i = 0; i < weekHoliday.length; i++) {
                ...
                String wdv = weekHoliday[i].split(" ")[1];  //节假日的 名称
