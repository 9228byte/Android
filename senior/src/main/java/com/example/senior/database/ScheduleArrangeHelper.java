...
public class ScheduleArrangeHelper extends DbHelper{
    ...
    public ScheduleArrangeHelper(Context context, String name, CursorFactory factory, int version) {
        ...
        mSelectedSQL = String.format("select _id,month,day,hour,minute,title,content,update_time,alarm_type from %s where ", mTableName);
    }
    ...
    public void update(ScheduleArrange data) {
        ...
        if (data.xuhao > 0) {
            update_sql = String.format("%s _id=%s;", update_sql, data.xuhao);
        } else
        {
            update_sql = String.format("%s day='%s';", update_sql, data.day);
        }
        Log.d(TAG, "update _sql: " + update_sql);
    ...
    public int getCount(String day) {
        String sql = String.format("%s day='%s';", mSelectedSQL, day);
    ...
    public List<?> queryInfoMonth(String month) {
        String sql = String.format("%s month='%s'", mSelectedSQL, month);
        return queryInfo(sql);
    }

    public List<?> queryInfoByDayRange(String begin_day, String end_day) {
        String sql = String.format("%s day>='%s' and day<='%s';",mSelectedSQL, begin_day, end_day);
        return queryInfo(sql);
    }
}
