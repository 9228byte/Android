...
/**
 * Created by lao on 2019/3/29
 */

public class UserDBHelper extends SQLiteOpenHelper {
    private static final String TAG = "UserDBHelper";
    private static final String DB_NAME = "user.db";    //s数据库的名称
    private static final int DB_VERSION = 1;        //数据库版本高
    ...
    public void onCreate(SQLiteDatabase db) {
        Log.d(TAG, "onCreate: ");
        String drop_sql = "DROP TABLE IF EXISTS " + TABLE_NAME  + ";";  //建表语句
        Log.d(TAG, "drop_sql：" + drop_sql);
        db.execSQL(drop_sql);   //执行删除表指令
        String create_sql = "CREATE TABLE IF NOT EXISTS " + TABLE_NAME + " ("
                +"_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,"
                + "name VARCHAR NOT NULL," + "age INTEGER NOT NULL,"
                + "height LONG NOT NULL," + "weight INTEGER NOT NULL,"
                + "married INTEGER NOT NULL," + "update_time VARCHAR NOT NULL"
                + ",phone VARCHAR" + ",password VARCHER"     //演示数据库升级时先把这行进行注释
                + ");";
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        Log.d(TAG, "onUpgrade oldVersion=" + oldVersion +", newVeersion=" + newVersion);
        if (newVersion > 1) {
           //Android的ALERT命令不支持一次添加多列，只能分多次添加
            String alter_sql = "ALTER TABLE " + TABLE_NAME + "ADD COLUM " + "phone VARCHAR;";
            Log.d(TAG, "alert_sql:" + alter_sql);
            db.execSQL(alter_sql);
            alter_sql = "ALTER TABLE " + TABLE_NAME + "ADD COLUM " + "password VARCHER";
    ...
    public int update(UserInfo info, String condition) {
        ...
        cv.put("married", info.married);
        cv.put("update_time", info.update_time);
        cv.put("phone", info.update_time);
        cv.put("password",info.password);
        //执行更新记录动作，该语句返回记录更新的shumu
        return mDB.update(TABLE_NAME, cv, condition, null);
    }

    public int update(UserInfo info) {
        //执行更新记录动作，该语句返回记录更新数目
        return update(info,"rowid=" + info.rowid);
    }

    //根据指定条件查询记录，并返回结果数据队列
    public ArrayList<UserInfo> qurey(String conditon) {
        String sql = String.format("select rowid,_id,name,age,height,weight,married,update_time," +
                "phone,password from %s where %s;", TABLE_NAME, conditon);
        Log.d(TAG, "qurey sql:" + sql);
        ArrayList<UserInfo> infoArray = new ArrayList<UserInfo>();
    ...
    public UserInfo queryByPhone(String phone) {
        UserInfo info = null;
        ArrayList<UserInfo> infoArray = qurey(String .format("phone='%s'",phone));
        if (infoArray.size() > 0) {
